...
def add_lang_dom_fields(doc: Dict[str, Any], language: str, domain: str) -> Dict:
...
def scrape_conditions(scraped_time_lang: str):
    """
...
def get_next_batch():
    ...
    with MongoConnection("places") as db:
        scraped_time_lang = f"review_extr_ds_{loc['language'].lower()}"
        cursor = db.collection.find(
            scrape_conditions(scraped_time_lang),
            {"place_id", "priority", "name", "data_id", scraped_time_lang}
            ).sort("priority", -1).limit(GMAPS_SCRAPE_BATCH_SIZE)
        documents = list(cursor)
        return [add_lang_dom_fields(doc, loc['language'], loc['domain']) 
